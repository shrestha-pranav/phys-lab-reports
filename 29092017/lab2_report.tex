...
%import pandas as pd
%
%m1 = pd.read_csv("phys_m_1.csv")
%p1 = pd.read_csv("phys_p_1.csv")
%m2 = pd.read_csv("phys_m_2.csv")
%p2 = pd.read_csv("phys_p_2.csv")
%
%meta = pd.read_csv("phys_meta.csv")
%m_error = 1 * 10**(-3)
%
%meta["h1"] /=100
%meta["h2"] /=100
...
%
%
%meta["hE"] = (10/7) * (meta["h2'"] + meta["h1"] - meta["h2"] - meta["h1'"])
%meta["hEerror"] = 20 * m_error / 7
%
%meta["diffD"] 
%    = m_error**2 * (2 * meta["h2"] * meta["hE"]) 
%    / (meta["L"]**2)
%meta["diffL"] 
%    =  m_error**2 * (2 * meta["D"]**2 * meta["h2"] * meta["hE"]) 
%    / (meta["L"]**4)
%meta["diffh2"] 
%    = m_error**2 * (meta["D"]**2 * meta["hE"])  
%    / ( 2 * meta["L"]**2 * meta["h2"])
%meta["diffhE"] 
%    = meta["hEerror"]**2 * (meta["D"]**2 * meta["h2"])  
%    / ( 2 * meta["L"]**2 * meta["hE"])
%
%meta["approxEror"] 
%    = (meta["diffD"] + meta["diffL"] + meta["diffh2"] + meta["diffhE"])**0.5
%meta["approx"] 
%    = (meta["D"] * (2 * meta["h2"] * meta["hE"])**0.5)/ (meta["L"])
%
%meta["v0"] = ((meta["hE"])*9.8)**0.5
%meta["vx"] = meta["v0"] * meta["D"] / meta["L"]
%meta["vy"] = meta["v0"] * (meta["h2"] - meta["h3"]) / meta["L"]
%meta["tf"] = (meta["vy"] + (meta["vy"]**2 + 2*9.8*meta["h2"]) ** 0.5) / 9.8
%meta["predict"] = meta["tf"] * meta["vx"]
%
%meta["predictError"] = meta["predict"] * meta["approxEror"] / meta["approx"]
%
%\end{lstlisting}
%\end{footnotesize}
